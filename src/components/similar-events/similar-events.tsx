...
interface EventSimilar {
  category: string;
  id: number;
}

export default async function SimilarEvents({ category, id }: EventSimilar) {
  const events = await db
    .select()
    .from(eventsDusseldorf)
    .where(and(eq(eventsDusseldorf.category, category), not(eq(eventsDusseldorf.id, id))));
  return (
    <section className="bg-border py-12">
      <div className="max-w-7xl ml-10 px-6">
        <h2 className="text-2xl font-semibold text-dark mb-6">Похожие события</h2>
